...
//        earthquakesURL = "quiz2.atom";
    ...
    private void addKey() {
        ...
        for (Marker em : quakeMarkers) {
            ((EarthquakeMarker) em).drawEarthquake(pg, em.getLocation().x, em.getLocation().y);
        }
    ...
    private void printQuakes() {
        ...
        String countryMarkerCountry = null;
        int oceanQuakeCounter = 0;
        for (Marker cm : countryMarkers) {
            if (cm.getProperties().size() > 0) {
                countryMarkerCountry = cm.getProperty("name").toString();
            }
            int landquakeCounter = 0;
            oceanQuakeCounter = 0;
            for (Marker qm : quakeMarkers) {
                if (qm.getClass() == LandQuakeMarker.class) {
                    String countryName = ((LandQuakeMarker) qm).getCountry();
                    if (countryName != null && Objects.equals(countryMarkerCountry, countryName)) {
                        landquakeCounter++;
                    }
                } else {
                    oceanQuakeCounter++;
            ...
            String s = MessageFormat.format("{0}: {1}", countryMarkerCountry, landquakeCounter);
            if (landquakeCounter > 0) {
                System.out.println(s);
            }
        }
        String ocenquakeString = MessageFormat.format("OCEAN QUAKES: {0}", oceanQuakeCounter);
        System.out.println(ocenquakeString);
    }
